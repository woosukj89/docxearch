    ...
    def search_words_together(self, word_doc, search_term):
        ...
        try:
            ...
            for p in ps:
                ...
                pattern = re.compile("\s*".join(words), re.IGNORECASE)
    ...
    def search_words_separately(self, word_doc, search_term):
        try:
            ...
            for p in ps:
                paragraph = Paragraph(p, None)
                words = search_term.split()
                if all(word.lower() in paragraph.text.lower() for word in words):
                    return word_doc
        except Exception:
            return None
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # self.highlight_checkbox.setGeometry(QRect(610, 40, 151, 31))
        # self.highlight_checkbox.setObjectName("highlight_checkbox")
        # self.highlight_checkbox.setText("highlight all occurences")
        # self.highlight_checkbox.stateChanged.connect(self.update_highlight_option)
    ...
    def display_paragraph(self):
        ...
        if self.word_search_option:
            for term in self.search_term.split():
                ...
                matches = regex.globalMatch(paragraph)
                while matches.hasNext():
                    match = matches.next()
                    if match.isValid:
                        text_cursor.setPosition(match.capturedStart())
                        text_cursor.movePosition(QTextCursor.Right, QTextCursor.KeepAnchor, len(term))
                        text_cursor.mergeCharFormat(format)
        else:
            pattern = "\\s*".join(self.search_term.split())
            ...
            while matches.hasNext():
                ...
                if match.isValid:
                    text_cursor.setPosition(match.capturedStart())
                    text_cursor.movePosition(QTextCursor.Right, QTextCursor.KeepAnchor, match.capturedLength())
                    text_cursor.mergeCharFormat(format)
